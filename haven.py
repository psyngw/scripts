...
def main(url):
    ...
    for pre in pres:
        ...
        if not os.path.isfile(os.path.join(STORE_PATH, pre_path)):
            img = requests.get("http://w.wallhaven.cc/full/%s/%s" % (name[:2], pre_path), verify=False)
...
if __name__ == '__main__':
    bp = input('please input the path you wanna save pics(or just Enter to save in the current folder):\n')
    if bp and bp[0] == '~':
        _b = os.popen('echo $HOME')
        bp = bp.replace('~', _b.read()[:-1], 1)
    BASE_PATH = bp or os.path.dirname(os.path.abspath(__file__))
    STORE_PATH = os.path.join(BASE_PATH, 'wallpapers', f'wallhaven-{datetime.datetime.now():%Y%m%d}')
    ...
    m_url = input('please input the pics\'url you wanna get(or just Enter to get random pics):\n')
    if m_url: url = m_url
    main(url)
